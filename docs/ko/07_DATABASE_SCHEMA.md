...
PostgreSQL 17 개발 DB, SQLAlchemy 2 모델, Alembic 마이그레이션, 명시적 개발 시드와 DB 테스트가 `backend/`에 구현되어 있다. 프론트엔드 `services/*.ts`는 아직 이 백엔드를 호출하지 않으므로 화면 데이터는 계속 React 메모리 mock을 사용한다.
...
현재 FastAPI에는 DB 연결 확인용 `GET /health`만 구현되어 있다.
