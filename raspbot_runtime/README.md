...
2026-09-02 확인한 SSH 주소는 `192.168.0.73`이며 아래 `.74`는 이전 접속 예시입니다.
접속 시 기기 정보를 확인하고, 실기 운전 전 센서·정지·바퀴 방향 시험을 진행하세요.
...
`localhost:3000` 왼쪽 메뉴에서 **수동 제어** 아래의 **라즈봇 리모컨**을 선택하면 방향 리모컨이 바로 열립니다. **라즈봇 · 집 지도**는 별도의 시뮬레이션 메뉴입니다. 리모컨은 이동 잠금으로 시작하며 다른 메뉴로 나가면 운전 권한을 잠그고, 보유한 운전 권한이 있을 때 정지를 요청합니다.
