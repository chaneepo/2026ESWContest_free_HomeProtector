...
## 최신 실행 절차 (2026-09-02, 안전 프로토콜 2)

이번 점검에서는 모터를 움직이지 않았습니다. [안전 점검과 제한](SAFETY_REVIEW.md)을
먼저 읽고, 첫 실물 시험은 네 바퀴를 띄우고 사람이 지켜보며 진행하세요.
이번 SSH 주소는 `.73`이었으며 아래 `.74`는 과거 예시입니다. 접속할 때 기기를 재확인합니다.

Pi의 SSH 터미널에서 실행:

```bash
cd /home/tracelab/chan
/home/tracelab/raspbot_ws/venv/bin/python server.py --host 127.0.0.1 --port 8090 --sensors-only
```

맥북의 **별도 터미널**에서 SSH 터널 실행 (주소가 바뀌면 수정):

```bash
ssh -N -L 8090:127.0.0.1:8090 tracelab@192.168.0.73
```

맥 브라우저에서 `http://127.0.0.1:8090`을 엽니다. 통합 웹앱은 `.env.local`의
`RASPBOT_API_URL=http://127.0.0.1:8090` 설정 후 개발 서버를 재시작합니다.

- 안전 확인 → 실기모드 순으로 누릅니다. I2C 읽기·정지 처리 실패 시 잠금이 유지됩니다.
- STOP, 창 비활성화, 통신 오류 뒤에는 다시 안전 확인해야 합니다. 두 화면에서 동시에 운전하지 마세요.
- 종료할 때 STOP 후 바퀴가 실제로 멈췄는지 확인하고 서버를 종료합니다.
- `--hardware`도 자동 운전 허용을 하지 않습니다. 무인 실행이나 인터넷 포트 공개는 하지 마세요.

아래 이전 기록보다 이 절의 안전 규칙을 우선합니다.
